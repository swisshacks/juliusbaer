...
def process_transcripts(json_folder, client_profiles):
	...
	with open('id_match_name.csv', 'w', newline='', encoding='utf-8') as csvfile:
...
names = client_profiles['name'].tolist()
...
df_id_match_name = pd.read_csv('id_match_name.csv')
df_id_match_name['name'] = df_id_match_name['name'].apply(lambda x: find_single_closest_match(x, names))

# Save the updated DataFrame
df_id_match_name.to_csv('../client_profiles/id_match_name_results.csv', index=False)